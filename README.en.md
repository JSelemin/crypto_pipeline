...
The pipeline follows a structured data engineering lifecycle:
...
3.  **Transformation**: DuckDB executes SQL-based analytical models, utilizing window functions and relational joins to produce final datasets.

```mermaid
graph LR
    A[CoinGecko API] --> B[Extract: Python/Requests]
    B --> C[(Staging: Parquet)]
    C --> D[Transform: DuckDB/SQL]
    D --> E[(Analyses: Parquet)]
```
